...
-- недеља, 23. децембар 2018. 20:44:53 CET
...
CREATE TABLE IF NOT EXISTS `veleprodaja`.`Ponuda` (
  ...
  `Cena` DECIMAL NULL,
  `Popust` DECIMAL NULL,
  `Prodajna cena` DECIMAL NULL,
  `Kontinuitet i broj isporuka` VARCHAR(45) NULL,
  `Nacin isporuke` VARCHAR(45) NULL,
  `Rok placanja` DATE NULL,
...
CREATE TABLE IF NOT EXISTS `veleprodaja`.`Izradjuje` (
  ...
  PRIMARY KEY (`Dokument_idDokument`, `Zaposleni_idZaposleni`),
  INDEX `fk_Dokument_has_Zaposleni_Zaposleni1_idx` (`Zaposleni_idZaposleni` ASC),
  INDEX `fk_Dokument_has_Zaposleni_Dokument1_idx` (`Dokument_idDokument` ASC),
  CONSTRAINT `fk_Dokument_has_Zaposleni_Dokument1`
    FOREIGN KEY (`Dokument_idDokument`)
    REFERENCES `veleprodaja`.`Dokument` (`idDokument`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_Dokument_has_Zaposleni_Zaposleni1`
    FOREIGN KEY (`Zaposleni_idZaposleni`)
    REFERENCES `veleprodaja`.`Zaposleni` (`idZaposleni`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
DELIMITER $$
CREATE TRIGGER testref BEFORE INSERT ON Porudzbina
  FOR EACH ROW
    begin
    ...
    INSERT INTO Dokument(VremeKreiranja,VremeIzmene,Status)
    VALUES (current_timestamp,current_timestamp,'radni');
    
    end$$
    
DELIMITER ;

DROP TRIGGER IF EXISTS testref2;
DELIMITER $$
CREATE TRIGGER testref2 BEFORE INSERT ON Ponuda
  FOR EACH ROW
    begin
...
DROP TRIGGER IF EXISTS testref3;
DELIMITER $$
CREATE TRIGGER testref3 BEFORE INSERT ON Otpremnica
  FOR EACH ROW
    begin
...
DROP TRIGGER IF EXISTS testref4;
DELIMITER $$
CREATE TRIGGER testref4 BEFORE INSERT ON Faktura
  FOR EACH ROW
    begin
